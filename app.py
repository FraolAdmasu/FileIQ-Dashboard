...
if page == "File & Folder Analysis":
    ...
    st.markdown("Upload a ZIP file or specify a server-accessible folder to generate insights.")
    ...
    with col2:
        st.write("Or scan a server folder")
        folder_path = st.text_input(
            "Enter folder path to scan (must be accessible by the app server):",
            value=st.session_state.get('scan_path', "")
        )
        if st.button("Scan Folder") and folder_path and os.path.isdir(folder_path):
